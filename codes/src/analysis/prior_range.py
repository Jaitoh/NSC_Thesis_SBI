...
for dur in dur_list:
    ...
    for i, seq in enumerate(seqs_divided[task_part[0]:task_part[1]]):
        print(f'chunk {i+1}/{len(seqs_divided)}')
    ...
    # save the output
    with open(f'../data/seqC_combinatorial_theta_dict_dur_{dur}_part{task_part[0]}_to_{task_part[1]}.pkl', 'wb') as f:
        pickle.dump(theta_collection, f)
    with open(f'../data/seqC_combinatorial_probR_dict_dur_{dur}_part{task_part[0]}_to_{task_part[1]}.pkl', 'wb') as f:
        pickle.dump(probR_collection, f)
