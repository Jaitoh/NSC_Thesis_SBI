...
class Feature_Generator:
    ...
    def compute_kernels(self, seqC, chR, D, M, S):
        ...
        self.dist_MD, self.dist_NS, self.stats_MD, self.stats_NS = self._compute_kernel_MD_NS(
                                    chR, 
                                    D, M, S, 
                                    Dur, MS, MD, nSwitch, 
                                    Dur_list, MS_list, MD_list
                                )
    ...
    def get_features(self, feature_list=[1,2,3,4,5]):
        ...
        y1s, y2s = [], []
        for i in range(self.M):
            y1, y2 = self._extract_y1_y2(self.stats_MD[:, i, :])
            y1s.append(y1)
            y2s.append(y2)
        ...
        y3s = []
        for i in range(3):
            y3s.append(self._extract_y3(self.stats_MD2[i, :], ranges=[-7, 7]))
        ...
        y4s = []
        for i in range(self.M):
            y4s.append(self._extract_y4(self.stats_NS[:, i, :]))
        
        # feature 5
        y5s = []
        for i in range(self.M):
            y5s.append(self._extract_y5(self.stats_psy[:, i, :]))
    ...
    def _extract_y5(self, stats_psy):
        D, P = stats_psy.shape
        y5 = []
        for i in range(P):
            starting_idx = i//2
    ...
    def _extract_y3(self, stats_MD2, ranges=[-7, 7]):
    ...
    def _extract_y1_y2(self, stats_MD):
    ...
    def _get_mask_NS(self, shape=(7, 21), ranges = [(-2, 2), (-4, 4), (-5, 5)], show_mask=True):
    ...
    def _get_mask_MD(self, shape=(7, 21), ranges = [(-2, 2), (-4, 4), (-5, 5)], show_mask=True):
    ...
    def plot_kernels(self, D, M, S, save_fig=False):
        stats_MD, dist_MD, stats_MD2, dist_MD2, stats_NS, dist_NS, stats_psy = self.stats_MD, self.dist_MD, self.stats_MD2, self.dist_MD2, self.stats_NS, self.dist_NS, self.stats_psy
...
def main():
    ...
    feature = FG.compute_kernels(seqC, chR, D, M, S).get_features()
