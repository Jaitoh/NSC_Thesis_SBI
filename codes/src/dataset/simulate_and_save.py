...
def simulate_and_save(data_path, config, seed):
    ...
    print(f"{os.path.getsize(data_path)/1024/1024} MB")

    return seqC, theta, probR


@hydra.main(
    config_path="../config", config_name="config-dataset-prior-v2", version_base=None
)
def main(config: DictConfig):
    ...
    PID = os.getpid()
    print(f"PID: {PID}")
    # log_file = f"{config.log_dir}/resource_usage_{config.run}.log"
    # monitor_process = multiprocessing.Process(
    #     target=monitor_resources, args=(PID, 5, log_file)
    # )
    ...
    simulate_and_save(config.data_path, config, config.seed)
