...
def monitor_resources(PID, interval, log_file):
    ...
    with open(log_file, "w") as f:
        print(
            f"Monitoring resources for PID {PID} every {interval} seconds and writing to {log_file}"
        )
        while True:
...
def print_cpu_memory_usage(prefix=""):
    ...
    print(
        f"{prefix} - cpu memory usage: {mem_info.rss / (1024 * 1024):.2f} MB", end=" | "
    )
...
def print_gpu_memory_usage():
    try:
        ...
        for gpu in gpu_stats:
            print(
                f"GPU {gpu.index} - Memory Used: {gpu.memory_used} MB / {gpu.memory_total} MB",
                end=" | ",
            )
