...
from neural_nets.embedding_nets import LSTM_Embedding, Transformer
...
class Solver:
    ...
    def get_neural_posterior(self):
        ...
        if config_density["embedding_net"]["type"] == "lstm":
            print(f"--- embedding net: LSTM ---")
        ...
        if config_density["embedding_net"]["type"] == "transformer":
            print(f"--- embedding net: Transformer ---")
            embedding_net = Transformer(
                dms=dms,
                l=l_x,
                hidden_size=config_density["embedding_net"]["hidden_size"],
                output_size=config_density["embedding_net"]["output_size"],
    ...
    def posterior_analysis(self, posterior, current_round, run):
        ...
        for fig_idx in tqdm(range(len(self.post_val_set["x"]))):
            ...
            plt.savefig(
                f"{self.log_dir}/posterior/post_plot_x_val_{fig_idx}_round{current_round}_run{run}.png"
            )
            plt.close(fig_x)
            ...
            plt.savefig(
                f"{self.log_dir}/posterior/post_plot_x_val_shuffled_{fig_idx}_round{current_round}_run{run}.png"
            )

        # save posterior for each round and run using pickle
        with open(
            f"{self.log_dir}/posterior/posterior_round{current_round}_run{run}.pkl",
            "wb",
        ) as f:
        ...
        for current_round in [
            ...
            for run in range(training_config["num_runs"]):
                ...
                self.inference, density_estimator = self.inference.train(
                    ...
                    resume_training=(run != 0)
                    or (current_round != 0),  # resume training if not the first run
                ...
                torch.save(
                    ...
                    f"{self.log_dir}/model/best_model_round{current_round}_run{run}.pt",
                )

                # print(f'finished training of === round {current_round} run {run} === in {(time.time()-start_time)/60:.2f} min\n\n')
                print(
                    f"finished training of === round {current_round} run {run} === in {(time.time()-start_time)/60:.2f} min\n\n"
                )
