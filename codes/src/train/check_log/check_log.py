...
def get_event_data(ea_post, ea_val, ea_train):
    ...
    best_epochs = np.array(
        [[e.wall_time, e.step, e.value] for e in ea_post.Scalars("run0/best_val_epoch_board")]
    )
...
def plot_lr_log_probs_(
    val_time, val_step, val_log_probs, train_step, train_log_probs, lr_step, lr_lr, best_epochs_epoch
):
    ...
    for i in range(len(best_epochs_epoch)):
        ax1.text(
            best_epochs_epoch[i],
            val_log_probs[best_epochs_epoch[i]] + 0.02,
            f"{val_log_probs[best_epochs_epoch[i]]:.2f}",
            color="red",
            fontsize=10,
            ha="center",
            va="bottom",
    ...
    ax2 = ax1.twiny()
    ax2.plot(
        (val_time - val_time[0]) / 60 / 60, min(val_log_probs) * np.ones_like(val_log_probs), "-", alpha=0
    )
...
def plot_log_prob(ax1, val_perf, train_perf, lr, best, plot_time=True):
    ...
    if isinstance(best_epochs_epoch, float):
        ...
        ax1.text(
            best_epochs_epoch,
            val_log_probs[int(best_epochs_epoch)] + 0.02,
            f"{val_log_probs[int(best_epochs_epoch)]:.2f}",
            color="red",
            fontsize=10,
            ha="center",
            va="bottom",
...
def plot_posterior_samples(log_dir, best_epochs_epoch, num_rows, exact_epoch=True):
    for case in ["train", "val"]:
        ...
        fig, axs = plt.subplots(
            num_rows, len(best_epochs_epoch), figsize=(len(best_epochs_epoch) * 2, num_rows * 2)
        )
...
def plot_one_img(chosen_plot_idx, plt_idx, plot_shuffled, log_dir, val_perf, train_perf, lr, best, exp_name):
...
def animate_posterior(
    log_dir, num_frames, plot_shuffled, val_perf, train_perf, lr, best, duration=1000, exp_name=""
):
    # extract figure information
    figures_all = os.listdir(log_dir / "posterior/figures")
    unique_val = np.unique(
        [
            fig.split("x_val_")[1].split("_epoch")[0]
            for fig in figures_all
            if fig.startswith("posterior_x_val")
        ]
    )

    unique_train = np.unique(
        [
            fig.split("x_train_")[1].split("_epoch")[0]
            for fig in figures_all
            if fig.startswith("posterior_x_train")
        ]
    )
    ...
    for plt_idx in tqdm(range(len(chosen_plot_idx))):
        fig = plot_one_img(
            chosen_plot_idx, plt_idx, plot_shuffled, log_dir, val_perf, train_perf, lr, best, exp_name
        )
...
if __name__ == "__main__":
    ...
    plot_shuffled = True
    animate_posterior(log_dir, num_frames, plot_shuffled, val_perf, train_perf, lr, best, duration, exp_name)
