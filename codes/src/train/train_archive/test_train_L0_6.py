...
nan2num = -1
num_probR_sample = 20  # 10 if test else 30
# check if the dataset exists
x_dir = Path('../data/training_datasets/x_test.pt') if test else Path('../data/training_datasets/x_15_6.pt')
theta_dir = Path('../data/training_datasets/theta_test.pt') if test else Path('../data/training_datasets/theta_15_6.pt')
...
x, theta = prepare_training_data_from_sampled_Rchoices(
    ...
    part_of_seqC=0.3,  # 700 -> 210
    # part_of_seqC=0.08,  # 700 -> 56
    part_of_prior=0.4,  # 500 -> 200,
    remove_sigma2i=True,
)
# save x, theta
torch.save(x, x_dir)
...
log_dir = Path('./src/train/logs_15_p0/log_test') if test else Path('./src/train/logs_15_p0/log_sample_Rchoices6')
