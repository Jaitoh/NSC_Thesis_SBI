...
class seqC_generator:
    ...
    def _generate_one(self, MS, dur) -> np.ndarray:  # shape (sample_size, dur_max-1)
        ...
        arr = MS * np.random.choice(
            [-1, 0, 1], size=(self.seqC_sample_per_MS, dur - 1), p=[self.pL, self.pP, self.pR]
        )
        ...
        while len(zero_rows) > 0:
            ...
            new = MS * np.random.choice(
                [-1, 0, 1], size=(len(zero_rows), dur - 1), p=[self.pL, self.pP, self.pR]
            )
...
def seqC_combinatorial_generator(dur):
    ...
    # remove the sequences with all zeros in a row
    seqs = np.array(seqs)
    zero_rows = np.where(~seqs.any(axis=1))[0]
    seqs = np.delete(seqs, zero_rows, axis=0)
