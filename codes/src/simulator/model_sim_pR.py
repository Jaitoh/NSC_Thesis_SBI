...
def DM_sim_for_seqCs_parallel(
    ...
    seqC = np.empty(
        (*seqCs.shape[:-1], params.shape[0], seqCs.shape[-1])
    )  # [dur_len, MS_len, sample_size, num_prior_sample, 15]
    theta = np.empty(
        (*seqCs.shape[:-1], *params.shape)
    )  # [dur_len, MS_len, sample_size, num_prior_sample, num_params(4)]
    probR = np.empty(
        (*seqCs.shape[:-1], params.shape[0], 1)
    )  # [dur_len, MS_len, sample_size, num_prior_sample, 1]
    print(
        f"total number of simulations",
        np.product(probR.shape),
...
def seqC_gen_and_DM_simulate(
    ...
    seqCs = seqC_gen.generate(
        dur_list=seqC_dur_list,
