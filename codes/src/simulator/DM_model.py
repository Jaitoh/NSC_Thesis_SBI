...
class DM_model:
    ...
    def stoch_simulation(self, seqC, debug=False, verbose=0):
        ...
        tempBins = np.arange(
            0, self.dt * (self.maxClength - 1), self.dt
        )  # in matlab, the code was tempBins = 0:dt:dt*(maxClength-2) including the maxClength-2 element 0-13dt
        orthogonalPolynomials = self.get_orthogonal_polynomials_curves(tempBins, order)

        orthogonalPolynomials = orthogonalPolynomials[
            :, : lenC - 1
        ]  # remove the first column of orthogonalPolynomials
        ...
        # stochastic cases
            ...
            if not np.all(np.isinf(B)):  # with (sticky) boundary condition
                ...
                assert (
                    probRchoice != np.nan
                ), f"probRchoice is nan with bias={self.bias}, a[-1]={a[-1]}, sigma2dt[-1]={sigma2dt[-1]}"
                if verbose != 0:
                    print(
                        f"==>> sigma2dt_last: {sigma2dt_last:.3f}, bias: {self.bias:.3f}, a[-1]: {a[-1]:.3f}"
                    )
        # === simulation ends ===
    ...
    def _compute_variance(self, seqC, dt, Lambda):
        ...
        for k in range(1, T):
            var_s[k] = (
                var_s[k - 1] * (1 + Lambda[k - 1] * dt) ** 2
                + seqC[k] ** 2 * dt**2 * self.temporalDiscretization * self.sigma2s
            )
    ...
    def _sensory_adaptation_for_seqC(self, seqC):
        ...
        if np.isnan(self.Selse).all():  # check if Selse is full of nan
            ...
            tempBins = np.arange(
                0, self.dt * (self.maxClength - 1), self.dt
            )  # in matlab, the code was tempDiscrBins = 0:dt:dt*(maxClength-2) including the maxClength-2 element 0-13dt
        ...
        tempBins = np.arange(
            0, self.dt * (self.maxClength - 2), self.dt
        )  # in matlab, the code was tempDiscrBins = 0:dt:dt*(maxClength-3) including the maxClength-3 element 0-12dt
    ...
    def _compute_mean_trace_fast(self, seqC, B, Lambda, dt):
        return cython_compute.compute_mean_trace_parallel(seqC, B, Lambda, dt)
    ...
    def _get_orthogonal_polynomials_curves(self, arrayIn, order):
        ...
        if order >= 6:
            matrixOut[6, :] = (
                1 / 16 * (231 * arrayInAffine**6 - 315 * arrayInAffine**4 + 105 * arrayInAffine**2 - 5)
            )
        if order >= 7:
            matrixOut[7, :] = (
                1
