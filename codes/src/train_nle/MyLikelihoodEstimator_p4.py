...
from train_nle.Dataset_v3 import x1pR_theta_Dataset
...
class MyLikelihoodEstimator(NeuralInference, ABC):
    ...
    def prepare_dataset_network(
        ...
        prefetch_factor = (
            config_dataset.prefetch_factor
            if (config_dataset.num_workers > 0) and (config_dataset.prefetch_factor > 0)
            else None
        )
        loader_kwargs = {
            "batch_size": min(
    ...
    def _loss(self, theta: Tensor, x: Tensor, pR: Tensor) -> Tensor:
        ...
        # abs loss
        return nn.L1Loss()(pR, torch.exp(log_prob))
    ...
    def __call__(self, theta: Tensor, track_gradients: bool = True) -> Tensor:
        ...
        with torch.set_grad_enabled(track_gradients):
            ...
            log_likelihood_trial_sum = (
                log_likelihood_trial_batch.reshape(self.x_o.shape[0], -1).sum(0).to(self.device)
            )
