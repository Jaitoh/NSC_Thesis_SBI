...
class RNN_avg(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(DM):
            ...
            sliced = sliced.permute(0, 2, 1)        # Shape: (B, L, S) - (B, 16, 700)
            # print(sliced.shape, x.shape)
            
            # gru1
            output = gru1(sliced)                   # Shape: (B, L, 256)
            # print(output.shape)
            
            # gru2
            output = gru2(output)                   # Shape: (B, L, 8)
            # print(output.shape)
            
            output = gru3(output)                   # Shape: (B, L, 1)
            
            output += output_r                      # Shape: (B, 1, L)
            output = self.bn3(output)               # Shape: (B, 1, L)
