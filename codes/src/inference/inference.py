...
setup_seed(0)
from dataset.dataset import process_x_seqC_part
...
class posterior_inference:
    def __init__(
        ...
        print(
            f"loading and processing trial data from {trial_data_path} for subject {subject_id} with trial length {trial_len}..."
        )
        ...
        self.posterior_samples = posterior_sampling(
            obs_data, n_sub_samples, posterior, posterior_sampling_save_path
        )
    ...
    def generate_one_hist(self, obs_data_idx=0):
        samples_sub = self.posterior_samples[
            self.n_sub_samples * obs_data_idx : self.n_sub_samples * (obs_data_idx + 1), :
        ]
    ...
    def gen_animation(self, ani_name, total_frames=100):
        ...
        ani = animation.FuncAnimation(
            fig,
            self._update,
            frames=np.linspace(0, self.obs_data.shape[0], total_frames, dtype=np.int),
            interval=5,
        )
