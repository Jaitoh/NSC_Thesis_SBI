...
class My_HighD_Sets(Dataset):
    def __init__(
        ...
        seqC_normed     of shape set_0:(DMS, 15)      - (14700, 15) #! add this part into the dataset (currently removed)
        ...
        probR           of shape (num_sets, DM, S, T) - (7, 21, 700, 5000) #! fixed probR sampling, in init function
        ...
        seqC_process, nan2num, summary_type = (
            dataset_config["seqC_process"],
            dataset_config["nan2num"],
            dataset_config["summary_type"],
        )
        ...
        randomly_chosen_theta_idx = np.random.choice(
        ...
        for set_idx, set_name in enumerate(chosen_set_names):
            ...
            probR_data = (
                f[set_name]["probR"][chosen_dur_idx, :, :, :][
                    :, :, :, randomly_chosen_theta_idx
                ]
                if crop_dur
                else f[set_name]["probR"][:, :, :, randomly_chosen_theta_idx]
            )  # (D, M, S, T)
    ...
    def __getitem__(self, idx):
        """#! probR sampling in init function
...
dataset = My_HighD_Sets(
    data_path,
    config,
    chosen_set_names=["set_0", "set_99"],
